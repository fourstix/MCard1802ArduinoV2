...
//character used for invalid key input
#define NO_KEY_CHAR -1
...
#define PGM_LIFE       6
#define PGM_IDIOT      7
#define PGM_GRAPHICS   8
#define PGM_TERMINAL   9
...
//Maximum size of string (without null)
#define STR_LENGTH  63

//States for Processing String
#define STR_LITERAL 0
#define STR_CHAR    1
#define STR_INPUT   2
#define STR_END     3
...
//Flag for literal Ascii character input
boolean literal_char = false;

//Flag for literal Ascii string input
boolean literal_string = false;

//Index for string byte
byte string_idx = 0;

//Status for processing string
byte str_status = STR_LITERAL;

//Buffer for string with Null
...
//Set the key buffer and data bus as if an ASCII character had been typed in
void setLiteral(char c_literal) {
  //Set key buffer as if an ASCII had been typed in as hex numbers
  key_buffer = c_literal;
  
  //set data value for mcp23017
  data_in = key_buffer & LOWER_BYTE_MASK;  
} //setLiteral 

//Check to see if we have a valid character
boolean isValidChar(char c_valid) {
  //Consider anything but No Key as valid
  return (c_valid != NO_KEY_CHAR);
} //isValidChar

//Check to see if we have an EOL character
boolean isEndOfLine(char c_eol) {
  //Carriage return or Newline
  return (c_eol == '\r' || c_eol == '\n');
} //isVEndOfLine

//Put all the characters in the String in buffer with null
void fillStringBuffer() {
  byte str_idx = 0;
  char c_next = NO_KEY_CHAR;
  //Flag for reaching end of input chracters
  boolean str_eol = false;

  while (Serial.available() > 0) {
    c_next = Serial.read();
    if (isEndOfLine(c_next)) {
      //No more characters in Serial Buffer after eol
      str_eol = true;
    } //if isEndOfLine(c_next)
    //Put characters in buffer until eol or buffer is filled
    if ((str_idx < STR_LENGTH) && !str_eol) {
      #if DEBUG
        Serial.print(str_idx);
        Serial.print(F(":"));
        Serial.println(c_next);
      #endif
      StrBuffer[str_idx++] = c_next;        
      } //if-else str_idx < STR_LENGTH && !str_eol
...
//Process a character from either the keypad or serial monitor as literal or normal input
void processChar(char c_new) {
  if (literal_char) {
    //Process this character as literal ASCII
    setLiteral(c_new);
    //Process next character as normal
    literal_char = false;
  } else {
    //Process character as normal input
    handleChar(c_new);
  } //if-else literal_char
} //processChar

//Handle character data from either the keypad or serial monitor
void handleChar(char c_data) {  
  //Handle character data as command or hexadecimal input
  switch(c_data) {        
    //Input
    case '#':
    case ',':
    ...
    case '%':
      //process the next character as a literal ASCII value
      literal_char = true;
    break;
    ...
    case '$':
      //process characters as ASCII values input as a String, until EOL
      beginStringProcess();
    break;
    ...
    default:         
      if (isHexadecimalDigit(c_data)) {
        #if DEBUG
          Serial.println(c_data);                  
        #endif
        processHexChar(c_data);
      }
       break;
  } // switch    
} //handleChar
...
void beginStringProcess() {
  //Fill buffer with remaining characters
  fillStringBuffer();

  //Reset string process variables  
  string_idx = 0;
  str_status = STR_LITERAL;
 
  //Set flag to process characters from string
  literal_string = true;      
} //beginStringProcess

//Process characters in a literal string
char processString() {
  char c_string = NO_KEY_CHAR;
  
  switch(str_status) {
    case STR_LITERAL:
      //Send a literal marker
      c_string = '%';
      str_status = STR_CHAR;
    break;

    case STR_CHAR:
      //Get the character from string buffer
      c_string = StrBuffer[string_idx++];
      //When we get a null, we are done
      if (c_string == 0) {
        str_status = STR_END;  
      } else {
        str_status = STR_INPUT;
      } // if-else c_string
    break;

    case STR_INPUT:
      //Send an input character
      c_string = ',';
      str_status = STR_LITERAL;
    break;  

    case STR_END:
      //Done. Send the last input
      c_string = '#';
...
void showMenu() {
  ...
  Serial.println(F(" #,I = Input data to memory during load mode. Multi-byte input: 00,01,02,03#"));  
  Serial.println(F("       Simulate an Input key press in run mode.")); 
  Serial.println(F(" % = ASCII literal, insert ASCII hex value of the next character"));
  Serial.println(F(" $ = ASCII string, insert ASCII hex values for each of up to 31 characters")); 
  Serial.println(F("     with inputs, until the End of Line. Then input a NULL (00) to end string."));
  Serial.println(F(" J = Jump to end of a page in examine mode."));
  Serial.println(F(" O = toggle state of Output control line /EF2"));
  Serial.println(F(" P = load a Program in load mode."));
  Serial.println(F(" S = change terminal Serial baud rate from 1200 baud to 2400 baud, "));  
  Serial.println(F("     then to 300 baud and then back to 1200."));
...
void showProgramMenu() {
  ...
  Serial.println(F(" 7 - load 4 pages with the IDIOT Monitor program"));  
  Serial.println(F(" 8 - load 5 pages with a Graphics Demo program"));  
  Serial.println(F(" 9 - load 5 pages with a basic TTY Terminal program"));  
}
...
//Set up the MCP23008 and MCP23017 GPIO port extenders
void setupPorts() {
  //Set up MCP23017's
  Wire.begin(); 
  mcpData.init();
 ...
 */ 
void setup() {
  //Set up the GPIO Port Extenders
  setupPorts();
...
void loop() {
  ...
  if ((jump_index < jump_count) && allow_serial) {
    ...
    //Else if we don't have a character from keypad, check the Serial input
    } else if (!isValidChar(c_input) && allow_serial) {
      if (literal_string) {
        c_input = processString();
      } else {
        c_input = checkSerial();
      } //if-else literal_string
    } //if terminal - else if !isValidChar && allow_serial
  } //if-else jump-index
