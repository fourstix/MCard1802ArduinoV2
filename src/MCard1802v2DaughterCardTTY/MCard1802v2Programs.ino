 ...
 * 
 * The original program source code are copyright their original authors.
 * These byte code arrays are compiled from their source code listings.
 * 
 */
...
//Program 0 - Tom Pittman's program 2.1 Memory Clear, modified to run in any page
const uint8_t clear_memory[] PROGMEM = {
  // This program loads zeros in all memory locations when it runs 
  0x90, 0xBE, 0xF8, 0x00, 0xAE, 0xEE, 0x73, 0x30, 0x05
};

//Program 1 - Tom Pittman's program 2.2 Memory Sequencer
const uint8_t sequence_memory[] PROGMEM = {
  // This program stores a sequence of numbers in memory. 
  0x90, 0xAE, 0xBE, 0xEE, 0x8E, 0x73, 0x30, 0x03
};
...
//Program 3 -- Tom Pittman's program 7.6 TV Digital clock 
...
//Program 4 - Quick video DMA test, based on Tom Pittman's program 7.1
const uint8_t video_dma_test[] PROGMEM = {
  0x90, 0xB1, 0xB2, 0xB3, 0xF8, 0x1B, 0xA1, 0xF8,
  0xFF, 0xA2, 0xF8, 0x0F, 0xA3, 0x71, 0x23, 0x69,
  0xF8, 0x18, 0xA0, 0x30, 0x10, 0x00, 0x00, 0x00,
  //Replace these values with any test pattern you prefer
...
//Program 7 - IDIOT Monitor
//  Written by Lee Hart
//
//  TMSI IDIOT Monitor Summary
//
...
//Program 8 - 64x64 Resolution Video Graphics Demo
//  Based on graphics code written by Richard Dienstknecht
//  This program shows text in 64x64 Video resolution, alternating with a Graphics Sprites demo when
//  the Input button is pressed. This program uses RAM memory 0x7A00 to 0x7F00 for video data.
const uint8_t graphics_demo[] PROGMEM = {
  //Address: 0x0000 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0xF8, 0x44, 0xA1, 0xF8, 0x00, 0xB1, 0xF8, 0xFF, 0xA2, 0xF8, 0x7E, 0xB2, 0xF8, 0xB1, 0xA3, 0xF8,
  0x02, 0xB3, 0xF8, 0x20, 0xA4, 0xF8, 0x00, 0xB4, 0xF8, 0x30, 0xA5, 0xF8, 0x00, 0xB5, 0xD3, 0xD3,
  0xE2, 0x96, 0x73, 0x86, 0x73, 0x93, 0xB6, 0x83, 0xA6, 0x46, 0xB3, 0x46, 0xA3, 0x30, 0x1F, 0xD3,
  0x96, 0xB3, 0x86, 0xA3, 0xE2, 0x12, 0x72, 0xA6, 0xF0, 0xB6, 0x30, 0x2F, 0xC4, 0x2F, 0x8F, 0x3A,
  
  //Address: 0x0040 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F    
  0x3C, 0xD5, 0x72, 0x70, 0xC4, 0x22, 0x78, 0x22, 0x52, 0xF8, 0x7A, 0xB0, 0xF8, 0x00, 0xA0, 0xC4,
  0xC4, 0xE2, 0x80, 0xE2, 0x20, 0xA0, 0xE2, 0x3C, 0x52, 0x80, 0xE2, 0x20, 0xA0, 0x34, 0x59, 0x30,
  0x42, 0xF8, 0x7A, 0xBE, 0xF8, 0x00, 0xAE, 0x4F, 0x5E, 0x1E, 0x8E, 0x3A, 0x67, 0xF8, 0x7B, 0x52,
  0x9E, 0xF5, 0x33, 0x67, 0xD5, 0xF8, 0x7A, 0xBE, 0xF8, 0x00, 0xAE, 0x9F, 0x5E, 0x1E, 0x8E, 0x3A,

  //Address: 0x0080 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F    
  0x7B, 0xF8, 0x7B, 0x52, 0x9E, 0xF5, 0x33, 0x7B, 0xD5, 0xF8, 0x7A, 0xBC, 0x9E, 0xFA, 0x3F, 0xFE,
  0xFE, 0xFE, 0xAC, 0x3B, 0x99, 0x9C, 0xFC, 0x01, 0xBC, 0x8C, 0x52, 0x8E, 0xFA, 0x3F, 0xF6, 0xF6,
  0xF6, 0xF4, 0xAC, 0x8E, 0xFA, 0x07, 0xBE, 0x8D, 0x32, 0xD9, 0xF8, 0x7B, 0x52, 0x9C, 0xF5, 0x3B,
  0xD9, 0x0F, 0xAB, 0xF8, 0x00, 0xBB, 0x2D, 0x1F, 0x9E, 0xAE, 0x8E, 0x32, 0xC6, 0x2E, 0x8B, 0xF6,
  
  //Address: 0x00C0 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0xAB, 0x9B, 0x76, 0xBB, 0x30, 0xBA, 0xEC, 0x8B, 0xF3, 0x5C, 0x1C, 0x9B, 0xF3, 0x5C, 0xE2, 0x8C,
  0xFC, 0x07, 0xAC, 0x9C, 0x7C, 0x00, 0xBC, 0x30, 0xA7, 0xD5, 0xD5, 0x0F, 0xAC, 0xC2, 0x01, 0x01,
  0x1F, 0x8F, 0x73, 0x9F, 0x73, 0x8E, 0x73, 0x9E, 0x73, 0xD4, 0x01, 0x02, 0x60, 0x72, 0xBE, 0xF0,
  0xAE, 0x8F, 0x73, 0x60, 0x8E, 0xF4, 0xFC, 0x01, 0xAE, 0x60, 0x72, 0xBF, 0xF0, 0xAF, 0xC0, 0x00,

  //Address: 0x0100 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0xDB, 0xD5, 0xF8, 0x00, 0xAF, 0xF8, 0x7E, 0xBF, 0xF8, 0x81, 0xAD, 0xF8, 0x01, 0xBD, 0x8C, 0xFF,
  0x20, 0xFA, 0xFE, 0xBC, 0xFE, 0x73, 0x60, 0x9C, 0xF4, 0x73, 0x60, 0xCB, 0x01, 0x22, 0x9D, 0xFC,
  0x01, 0xBD, 0x8D, 0xF4, 0xAD, 0x3B, 0x2B, 0x9D, 0xFC, 0x01, 0xBD, 0x0D, 0xF6, 0xF6, 0xF6, 0xF6,
  0x73, 0x60, 0x8C, 0xFA, 0x01, 0xBC, 0xCA, 0x01, 0x4C, 0xF0, 0xAB, 0xF8, 0x00, 0xBB, 0x9B, 0xF6,
  
  //Address: 0x0140 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F      
  0xF9, 0x80, 0xBB, 0x2B, 0x8B, 0xCA, 0x01, 0x3E, 0xF0, 0x73, 0x30, 0x52, 0xF0, 0xBB, 0x0D, 0xFA,
  0x07, 0x73, 0x1D, 0xF8, 0x05, 0xAC, 0x0D, 0x73, 0x60, 0x1D, 0x9C, 0x3A, 0x63, 0x9B, 0xF2, 0x73,
  0x60, 0x30, 0x6D, 0x9B, 0xAB, 0xF0, 0xFE, 0x73, 0x60, 0x2B, 0x8B, 0x3A, 0x65, 0xF0, 0x5F, 0x1F,
  0x2C, 0x8C, 0x3A, 0x56, 0xF8, 0x00, 0xAF, 0xF8, 0x05, 0xAD, 0xD4, 0x00, 0x89, 0x60, 0xF0, 0xAF,
  
  //Address: 0x0180 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F        
  0xD5, 0x11, 0x40, 0x40, 0x40, 0x00, 0x40, 0x35, 0xAA, 0xBF, 0x0A, 0x1F, 0x0A, 0x33, 0x74, 0xC4,
  0x48, 0x70, 0xD4, 0x41, 0x48, 0xA8, 0x40, 0xA0, 0xD0, 0x22, 0x60, 0x90, 0x90, 0x90, 0x60, 0x33,
  0x00, 0xA8, 0x5C, 0xA8, 0x00, 0x22, 0x00, 0x00, 0x30, 0x40, 0x80, 0x13, 0x10, 0x10, 0x20, 0x40,
  0xC0, 0x33, 0x48, 0xB8, 0xA8, 0xA8, 0x5C, 0x33, 0xD8, 0x24, 0x48, 0x84, 0xF8, 0x33, 0x3C, 0xB0,
  
  //Address: 0x01C0 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F        
  0xF8, 0x24, 0x38, 0x33, 0x5C, 0x84, 0xC4, 0xA8, 0x48, 0x33, 0x48, 0xB4, 0x4C, 0xA4, 0x48, 0x12,
  0x00, 0x20, 0x80, 0x20, 0xC0, 0x32, 0x20, 0x58, 0x80, 0x58, 0x20, 0x33, 0x88, 0x54, 0x24, 0x48,
  0x88, 0x33, 0x48, 0xF4, 0x9C, 0xB4, 0x54, 0x33, 0xCC, 0xB0, 0xD0, 0xB0, 0xCC, 0x33, 0xDC, 0xB0,
  0xB8, 0xB0, 0xDC, 0x33, 0xEC, 0x90, 0xD0, 0x94, 0x8C, 0x33, 0xBC, 0xA8, 0xE8, 0xA8, 0xBC, 0x33,
  
  //Address: 0x0200 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F    
  0x34, 0x34, 0x38, 0xB4, 0x54, 0x35, 0x91, 0x9B, 0x95, 0x91, 0xF1, 0x43, 0x9E, 0xDA, 0xBA, 0x9A,
  0x9E, 0x34, 0xDE, 0xB2, 0xD2, 0x96, 0x9E, 0x33, 0xCC, 0xB0, 0xC8, 0xA4, 0xB8, 0x33, 0xF4, 0x54,
  0x54, 0x54, 0x5C, 0x35, 0xB1, 0xB1, 0xB1, 0xB5, 0x4A, 0x33, 0xB4, 0xB4, 0x48, 0xA8, 0xA8, 0x32,
  0xF8, 0x30, 0x50, 0x90, 0xF8, 0x32, 0x98, 0x88, 0x48, 0x28, 0x38, 0x33, 0x40, 0xA0, 0x00, 0x00,
  
  //Address: 0x0240 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F      
  0x1C, 0x23, 0x40, 0x98, 0x28, 0x28, 0x18, 0x33, 0x80, 0xCC, 0xB0, 0xB0, 0xCC, 0x33, 0x20, 0x6C,
  0xB4, 0xB8, 0x6C, 0x23, 0x58, 0xA8, 0xF0, 0x88, 0xB0, 0x31, 0x90, 0x80, 0xD0, 0xB0, 0xB0, 0x23,
  0x60, 0x28, 0x70, 0x68, 0xA8, 0x25, 0x80, 0x94, 0xAA, 0xAA, 0x6A, 0x33, 0x00, 0xC8, 0xB4, 0xB4,
  0xA8, 0x33, 0x00, 0xCC, 0xB4, 0xCC, 0x84, 0x23, 0x00, 0x58, 0xB0, 0x88, 0xB0, 0x23, 0x80, 0xE8,

  //Address: 0x0280 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F        
  0xA8, 0xA8, 0x58, 0x35, 0x00, 0xB1, 0xB5, 0xB5, 0x4A, 0x33, 0x00, 0xB4, 0x4C, 0x44, 0xA8, 0x33,
  0x0C, 0xE8, 0x30, 0x48, 0xEC, 0x13, 0xE0, 0xA0, 0x90, 0xA0, 0xE0, 0x44, 0x05, 0x5A, 0xA5, 0x0A,
  0x05, 0x9F, 0xFC, 0x01, 0xBF, 0x3F, 0xA1, 0x37, 0xA7, 0x6C, 0xAF, 0xD5, 0x8F, 0x52, 0x64, 0x22,
  0xD5, 0xE2, 0xF8, 0x00, 0xBF, 0xD4, 0x00, 0x75, 0xD4, 0x00, 0xDA, 0xD4, 0x00, 0xDA, 0xD4, 0x02,
  
  //Address: 0x02C0 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F          
  0xC6, 0xD4, 0x03, 0x4D, 0x30, 0xBE, 0xF8, 0x00, 0xBF, 0xD4, 0x00, 0x75, 0xF8, 0x04, 0xBF, 0xF8,
  0x5D, 0xAF, 0xF8, 0x04, 0xAE, 0xF8, 0x00, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0x6C,
  0xAF, 0xF8, 0x00, 0xAE, 0xF8, 0x12, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0x7D, 0xAF,
  0xF8, 0x00, 0xAE, 0xF8, 0x18, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0x8E, 0xAF, 0xF8,
  
  //Address: 0x0300 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F            
  0x00, 0xAE, 0xF8, 0x1E, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0x9E, 0xAF, 0xF8, 0x00,
  0xAE, 0xF8, 0x24, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0xAE, 0xAF, 0xF8, 0x00, 0xAE,
  0xF8, 0x2A, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0xC0, 0xAF, 0xF8, 0x00, 0xAE, 0xF8,
  0x30, 0xBE, 0xD4, 0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0xDE, 0xAF, 0xF8, 0x08, 0xAE, 0xF8, 0x3A,
  
  //Address: 0x0340 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F              
  0xBE, 0xD4, 0x00, 0xDB, 0xD4, 0x00, 0xDA, 0x69, 0xD4, 0x02, 0xA1, 0x61, 0xD5, 0xF8, 0x00, 0xBF,
  0xD4, 0x00, 0x75, 0xF8, 0x08, 0xAE, 0xF8, 0x07, 0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x0A, 0xAF, 0xF8,
  0x0F, 0xAD, 0xD4, 0x00, 0x89, 0xF8, 0x10, 0xAE, 0xF8, 0x00, 0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x19,
  0xAF, 0xF8, 0x0E, 0xAD, 0xD4, 0x00, 0x89, 0xF8, 0x18, 0xAE, 0xF8, 0x03, 0xBE, 0xF8, 0x04, 0xBF,

  //Address: 0x0380 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F              
  0xF8, 0x27, 0xAF, 0xF8, 0x0B, 0xAD, 0xD4, 0x00, 0x89, 0xF8, 0x20, 0xAE, 0xF8, 0x0C, 0xBE, 0xF8,
  0x04, 0xBF, 0xF8, 0x32, 0xAF, 0xF8, 0x0A, 0xAD, 0xD4, 0x00, 0x89, 0xF8, 0x20, 0xAE, 0xF8, 0x20,
  0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x3C, 0xAF, 0xF8, 0x0B, 0xAD, 0xD4, 0x00, 0x89, 0xF8, 0x28, 0xAE,
  0xF8, 0x25, 0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x47, 0xAF, 0xF8, 0x08, 0xAD, 0xD4, 0x00, 0x89, 0xF8,

  //Address: 0x03C0 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F                
  0x30, 0xAE, 0xF8, 0x26, 0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x4F, 0xAF, 0xF8, 0x06, 0xAD, 0xD4, 0x00,
  0x89, 0xF8, 0x38, 0xAE, 0xF8, 0x20, 0xBE, 0xF8, 0x04, 0xBF, 0xF8, 0x55, 0xAF, 0xF8, 0x08, 0xAD,
  0xD4, 0x00, 0x89, 0xF8, 0x04, 0xBF, 0xF8, 0xD1, 0xAF, 0xF8, 0x0D, 0xAE, 0xF8, 0x32, 0xBE, 0xD4,
  0x00, 0xDB, 0xF8, 0x04, 0xBF, 0xF8, 0xDE, 0xAF, 0xF8, 0x08, 0xAE, 0xF8, 0x38, 0xBE, 0xD4, 0x00,
  
  //Address: 0x0400 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F                  
  0xDB, 0xD4, 0x00, 0xDA, 0x69, 0xD4, 0x02, 0xA1, 0x61, 0xD5, 0x01, 0x03, 0x07, 0x0F, 0x0F, 0x1F,
  0x38, 0x70, 0xE0, 0xC0, 0xC0, 0xC0, 0x60, 0x60, 0x20, 0x06, 0x06, 0x0F, 0x7F, 0x7F, 0x7F, 0x1F,
  0xDF, 0xD9, 0xC9, 0xE6, 0xF0, 0xFF, 0x1F, 0xE0, 0xE0, 0xE0, 0x80, 0xB8, 0xBC, 0x3E, 0x7F, 0xFF,
  0xFF, 0x81, 0x80, 0xC0, 0xE0, 0x70, 0x30, 0x30, 0x30, 0x30, 0x60, 0x40, 0x60, 0xF0, 0xF0, 0xF0,

  //Address: 0x0440 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F                  
  0x60, 0x70, 0x38, 0x1C, 0x0F, 0x07, 0x03, 0x06, 0x0F, 0x7F, 0xFF, 0xFF, 0xF9, 0x39, 0x0F, 0x01,
  0xE3, 0xFF, 0xFE, 0xFC, 0xC0, 0x60, 0xF0, 0xF0, 0xF0, 0x60, 0xE0, 0xC0, 0x80, 0x36, 0x34, 0x78,
  0x36, 0x34, 0x20, 0x52, 0x65, 0x73, 0x20, 0x44, 0x65, 0x6D, 0x6F, 0x00, 0x20, 0x21, 0x22, 0x23,
  0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x5C, 0x00, 0x30, 0x31, 0x32,
  
  //Address: 0x0480 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F                    
  0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x00, 0x40, 0x41,
  0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x00, 0x4F, 0x50,
  0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5B, 0x2F, 0x5D, 0x00, 0x5E, 0x5F,
  0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x00,

  //Address: 0x04C0 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F                      
  0x6F, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E,
  0x00, 0x42, 0x69, 0x67, 0x20, 0x53, 0x70, 0x72, 0x69, 0x74, 0x65, 0x73, 0x21, 0x00, 0x28, 0x50,
  0x72, 0x65, 0x73, 0x73, 0x20, 0x49, 0x6E, 0x70, 0x75, 0x74, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  };

//Program 9 - 64x64 Teletype Terminal Program
//  This program displays text entered on the hexpad input as ASCIi characters.  Text typed in will 
//  advance the cursor and wrap around to the next line. It supports ten lines of about 16 average
//  width characters, and a basic set of control characters to move the text cursor.
//
//  Newline (0D) and Carriage Return (0A) to start text on a new line.  Tab (09) advances the cursor
//  to the next tab position.  Vertical Tab (0B) moves the cursor down to the next line at the 
//  same horizontal position.  FormFeed (0C) will clear the screen and home the cursor.
//  Cancel (18) erases an entire line and moves the cursor back to the beginning of the line.
//  Backspace (08) moves the cursor back the average character width and deletes the character
//  pixels for an average width character.  The Delete character (7F) moves back one pixel and deletes
//  one column of character pixels and Unit Space (1F) advances the cursor one pixel width. 
//
//  This program uses RAM memory 0x7D00 to 0x7F80 for video data.
//
const uint8_t tty_terminal[] PROGMEM = {
  //Address: 0x0000 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xF8, 0x44, 0xA1, 0xF8, 0x00, 0xB1, 0xF8, 0x7F, 0xA2, 0xF8, 0x7F, 0xB2, 0xF8, 0xD7, 0xA3, 0xF8,
  0x04, 0xB3, 0xF8, 0x20, 0xA4, 0xF8, 0x00, 0xB4, 0xF8, 0x30, 0xA5, 0xF8, 0x00, 0xB5, 0xD3, 0xD3,
  0xE2, 0x96, 0x73, 0x86, 0x73, 0x93, 0xB6, 0x83, 0xA6, 0x46, 0xB3, 0x46, 0xA3, 0x30, 0x1F, 0xD3,
  0x96, 0xB3, 0x86, 0xA3, 0xE2, 0x12, 0x72, 0xA6, 0xF0, 0xB6, 0x30, 0x2F, 0xC4, 0x2F, 0x8F, 0x3A,
  
  //Address: 0x0040 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0x3C, 0xD5, 0x72, 0x70, 0xC4, 0x22, 0x78, 0x22, 0x52, 0xF8, 0x7D, 0xB0, 0xF8, 0x00, 0xA0, 0xC4,
  0xC4, 0xE2, 0x80, 0xE2, 0x20, 0xA0, 0xE2, 0x3C, 0x52, 0x80, 0xE2, 0x20, 0xA0, 0x34, 0x59, 0x30,
  0x42, 0xF8, 0x7D, 0xBE, 0xF8, 0x00, 0xAE, 0x4F, 0x5E, 0x1E, 0x8E, 0x3A, 0x67, 0xF8, 0x7E, 0x52,
  0x9E, 0xF5, 0x33, 0x67, 0xD5, 0xF8, 0x7D, 0xBE, 0xF8, 0x00, 0xAE, 0x9F, 0x5E, 0x1E, 0x8E, 0x3A,

  //Address: 0x0080 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0x7B, 0xF8, 0x7E, 0x52, 0x9E, 0xF5, 0x33, 0x7B, 0xD5, 0xF8, 0x7D, 0xBC, 0x9E, 0xFA, 0x3F, 0xFE,
  0xFE, 0xFE, 0xAC, 0x3B, 0x99, 0x9C, 0xFC, 0x01, 0xBC, 0x8C, 0x52, 0x8E, 0xFA, 0x3F, 0xF6, 0xF6,
  0xF6, 0xF4, 0xAC, 0x8E, 0xFA, 0x07, 0xBE, 0x8D, 0x32, 0xD9, 0xF8, 0x7E, 0x52, 0x9C, 0xF5, 0x3B,
  0xD9, 0x0F, 0xAB, 0xF8, 0x00, 0xBB, 0x2D, 0x1F, 0x9E, 0xAE, 0x8E, 0x32, 0xC6, 0x2E, 0x8B, 0xF6,
  
  //Address: 0x00C0 (Page 0)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F  
  0xAB, 0x9B, 0x76, 0xBB, 0x30, 0xBA, 0xEC, 0x8B, 0xF3, 0x5C, 0x1C, 0x9B, 0xF3, 0x5C, 0xE2, 0x8C,
  0xFC, 0x07, 0xAC, 0x9C, 0x7C, 0x00, 0xBC, 0x30, 0xA7, 0xD5, 0xD5, 0x0F, 0xAC, 0x32, 0xFF, 0x1F,
  0x8F, 0x73, 0x9F, 0x73, 0x8E, 0x73, 0x9E, 0x73, 0xD4, 0x01, 0x00, 0x60, 0x72, 0xBE, 0xF0, 0xAE,
  0x8F, 0x73, 0x60, 0x8E, 0xF4, 0xFC, 0x01, 0xAE, 0x60, 0x72, 0xBF, 0xF0, 0xAF, 0x30, 0xDB, 0xD5,

  //Address: 0x0100 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xF8, 0x00, 0xAF, 0xF8, 0x7F, 0xBF, 0xF8, 0x7C, 0xAD, 0xF8, 0x01, 0xBD, 0x8C, 0xFF, 0x20, 0xFA,
  0xFE, 0xBC, 0xFE, 0x73, 0x60, 0x9C, 0xF4, 0x73, 0x60, 0x3B, 0x1F, 0x9D, 0xFC, 0x01, 0xBD, 0x8D,
  0xF4, 0xAD, 0x3B, 0x28, 0x9D, 0xFC, 0x01, 0xBD, 0x0D, 0xF6, 0xF6, 0xF6, 0xF6, 0x73, 0x60, 0x8C,
  0xFA, 0x01, 0xBC, 0x3A, 0x47, 0xF0, 0xAB, 0xF8, 0x00, 0xBB, 0x9B, 0xF6, 0xF9, 0x80, 0xBB, 0x2B,

  //Address: 0x0140 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x8B, 0x3A, 0x3A, 0xF0, 0x73, 0x30, 0x4D, 0xF0, 0xBB, 0x0D, 0xFA, 0x07, 0x73, 0x1D, 0xF8, 0x05,
  0xAC, 0x0D, 0x73, 0x60, 0x1D, 0x9C, 0x3A, 0x5E, 0x9B, 0xF2, 0x73, 0x60, 0x30, 0x68, 0x9B, 0xAB,
  0xF0, 0xFE, 0x73, 0x60, 0x2B, 0x8B, 0x3A, 0x60, 0xF0, 0x5F, 0x1F, 0x2C, 0x8C, 0x3A, 0x51, 0xF8,
  0x00, 0xAF, 0xF8, 0x05, 0xAD, 0xD4, 0x00, 0x89, 0x60, 0xF0, 0xAF, 0xD5, 0x11, 0x40, 0x40, 0x40,
  
  //Address: 0x0180 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x00, 0x40, 0x35, 0xAA, 0xBF, 0x0A, 0x1F, 0x0A, 0x33, 0x74, 0xC4, 0x48, 0x70, 0xD4, 0x41, 0x48,
  0xA8, 0x40, 0xA0, 0xD0, 0x22, 0x60, 0x90, 0x90, 0x90, 0x60, 0x33, 0x00, 0xA8, 0x5C, 0xA8, 0x00,
  0x22, 0x00, 0x00, 0x30, 0x40, 0x80, 0x13, 0x10, 0x10, 0x20, 0x40, 0xC0, 0x33, 0x48, 0xB8, 0xA8,
  0xA8, 0x5C, 0x33, 0xD8, 0x24, 0x48, 0x84, 0xF8, 0x33, 0x3C, 0xB0, 0xF8, 0x24, 0x38, 0x33, 0x5C,
  
  //Address: 0x01C0 (Page 1)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x84, 0xC4, 0xA8, 0x48, 0x33, 0x48, 0xB4, 0x4C, 0xA4, 0x48, 0x12, 0x00, 0x20, 0x80, 0x20, 0xC0,
  0x32, 0x20, 0x58, 0x80, 0x58, 0x20, 0x33, 0x88, 0x54, 0x24, 0x48, 0x88, 0x33, 0x48, 0xF4, 0x9C,
  0xB4, 0x54, 0x33, 0xCC, 0xB0, 0xD0, 0xB0, 0xCC, 0x33, 0xDC, 0xB0, 0xB8, 0xB0, 0xDC, 0x33, 0xEC,
  0x90, 0xD0, 0x94, 0x8C, 0x33, 0xBC, 0xA8, 0xE8, 0xA8, 0xBC, 0x33, 0x34, 0x34, 0x38, 0xB4, 0x54,
  
  //Address: 0x0200 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x35, 0x91, 0x9B, 0x95, 0x91, 0xF1, 0x43, 0x9E, 0xDA, 0xBA, 0x9A, 0x9E, 0x34, 0xDE, 0xB2, 0xD2,
  0x96, 0x9E, 0x33, 0xCC, 0xB0, 0xC8, 0xA4, 0xB8, 0x33, 0xF4, 0x54, 0x54, 0x54, 0x5C, 0x35, 0xB1,
  0xB1, 0xB1, 0xB5, 0x4A, 0x33, 0xB4, 0xB4, 0x48, 0xA8, 0xA8, 0x32, 0xF8, 0x30, 0x50, 0x90, 0xF8,
  0x32, 0x98, 0x88, 0x48, 0x28, 0x38, 0x33, 0x40, 0xA0, 0x00, 0x00, 0x1C, 0x23, 0x40, 0x98, 0x28,
  
  //Address: 0x0240 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x28, 0x18, 0x33, 0x80, 0xCC, 0xB0, 0xB0, 0xCC, 0x33, 0x20, 0x6C, 0xB4, 0xB8, 0x6C, 0x23, 0x58,
  0xA8, 0xF0, 0x88, 0xB0, 0x31, 0x90, 0x80, 0xD0, 0xB0, 0xB0, 0x23, 0x60, 0x28, 0x70, 0x68, 0xA8,
  0x25, 0x80, 0x94, 0xAA, 0xAA, 0x6A, 0x33, 0x00, 0xC8, 0xB4, 0xB4, 0xA8, 0x33, 0x00, 0xCC, 0xB4,
  0xCC, 0x84, 0x23, 0x00, 0x58, 0xB0, 0x88, 0xB0, 0x23, 0x80, 0xE8, 0xA8, 0xA8, 0x58, 0x35, 0x00,
  
  //Address: 0x0280 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xB1, 0xB5, 0xB5, 0x4A, 0x33, 0x00, 0xB4, 0x4C, 0x44, 0xA8, 0x33, 0x0C, 0xE8, 0x30, 0x48, 0xEC,
  0x13, 0xE0, 0xA0, 0x90, 0xA0, 0xE0, 0x41, 0x00, 0x50, 0xA0, 0x00, 0x00, 0xE2, 0xF8, 0x00, 0xAF,
  0xD4, 0x02, 0xF5, 0xD4, 0x03, 0xE7, 0xD5, 0x69, 0xF8, 0xFF, 0xAF, 0xD4, 0x02, 0xF5, 0xD5, 0x61,
  0x22, 0xF8, 0x00, 0xAF, 0xD4, 0x02, 0xF5, 0xD5, 0xF8, 0x07, 0xAD, 0xF8, 0x7F, 0xBD, 0x0D, 0x32,
  
  //Address: 0x02C0 (Page 2)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xCE, 0x34, 0xC1, 0xF8, 0x14, 0x3C, 0xC5, 0xFF, 0x02, 0x34, 0xC7, 0xFE, 0x33, 0xC3, 0xD5, 0xF8,
  0x7D, 0xBF, 0x9E, 0xFA, 0x3F, 0xFE, 0xFE, 0xFE, 0xAF, 0x3B, 0xDF, 0x9F, 0xFC, 0x01, 0xBF, 0xD5,
  0x8E, 0xFA, 0x07, 0xAC, 0x8E, 0xFA, 0x3F, 0xF6, 0xF6, 0xF6, 0xBC, 0x73, 0x60, 0x8F, 0xF4, 0xAF,
  0x9F, 0x7C, 0x00, 0xBF, 0xD5, 0xF8, 0x07, 0xAD, 0xF8, 0x7F, 0xBD, 0x8F, 0x5D, 0xD5, 0xD4, 0x02,
  
  //Address: 0x0300 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xCF, 0xF8, 0x00, 0xBD, 0xAD, 0xF8, 0x00, 0x5F, 0x1F, 0x1D, 0x8D, 0xFD, 0x40, 0xCA, 0x03, 0x05,
  0xD5, 0x8E, 0xFC, 0x08, 0xFA, 0x78, 0xAE, 0xFD, 0x38, 0x33, 0x1E, 0xD4, 0x03, 0x3C, 0xD5, 0xF8,
  0x04, 0xAF, 0xD4, 0x03, 0x8F, 0xD4, 0x03, 0x5A, 0xD5, 0xF8, 0xFF, 0xBD, 0x8C, 0xAD, 0x32, 0x37,
  0x9D, 0xF6, 0xBD, 0x2D, 0x8D, 0x30, 0x2E, 0x9D, 0xFB, 0xFF, 0xBD, 0xD5, 0xF8, 0x00, 0xAE, 0x9E,
  
  //Address: 0x0340 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xFC, 0x06, 0xBE, 0xFD, 0x3C, 0x33, 0x4A, 0xF8, 0x02, 0xBE, 0xD5, 0x9E, 0xFC, 0x06, 0xBE, 0xFD,
  0x3C, 0x33, 0x56, 0xF8, 0x02, 0xBE, 0xD4, 0x02, 0xFE, 0xD5, 0xD4, 0x02, 0xCF, 0xD4, 0x02, 0xE0,
  0x8E, 0x3A, 0x68, 0xD4, 0x02, 0xFE, 0x30, 0x8E, 0xD4, 0x03, 0x29, 0xF8, 0x00, 0xAD, 0x9D, 0x73,
  0x60, 0x0F, 0xF2, 0x5F, 0x9C, 0xFD, 0x07, 0x32, 0x7E, 0xF8, 0x00, 0x1F, 0x5F, 0x2F, 0x1D, 0x8D,
  
  //Address: 0x0380 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xFD, 0x05, 0x32, 0x8E, 0x8F, 0xFC, 0x08, 0xAF, 0x9F, 0x7C, 0x00, 0xBF, 0x30, 0x6E, 0xD5, 0x8E,
  0x32, 0x9E, 0x73, 0x60, 0x8F, 0xF5, 0xAE, 0x33, 0xB3, 0xF8, 0x00, 0xAE, 0x30, 0xB3, 0x9E, 0xFF,
  0x06, 0xBE, 0x3B, 0xAD, 0xF8, 0x40, 0x73, 0x60, 0x8F, 0xF5, 0xAE, 0x30, 0xB3, 0xF8, 0x02, 0xBE,
  0xF8, 0x00, 0xAE, 0xD5, 0x8F, 0x73, 0x60, 0x8E, 0xF4, 0xFC, 0x01, 0xAE, 0xFD, 0x3C, 0x33, 0xCE,
  
  //Address: 0x03C0 (Page 3)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xF8, 0x00, 0xAE, 0x9E, 0xFC, 0x06, 0xBE, 0xFD, 0x3C, 0x33, 0xCE, 0xF8, 0x02, 0xBE, 0xD5, 0xF8,
  0x01, 0xAF, 0xD4, 0x03, 0x8F, 0xD4, 0x03, 0x5A, 0xD5, 0xF8, 0x00, 0xAF, 0xD4, 0x03, 0xB4, 0xD5,
  0xF8, 0x00, 0xAE, 0xD4, 0x02, 0xFE, 0xD5, 0xD4, 0x02, 0xB8, 0xF8, 0x00, 0xBF, 0xD4, 0x00, 0x75,
  0xF8, 0x00, 0xAE, 0xF8, 0x02, 0xBE, 0xD4, 0x04, 0xB5, 0xD5, 0x3F, 0xFA, 0x6C, 0xFA, 0x7F, 0xAC,

  //Address: 0x0400 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x37, 0x00, 0xD5, 0x8C, 0x52, 0x64, 0x22, 0xD5, 0x3F, 0x08, 0x6C, 0xAC, 0x37, 0x0C, 0xD5, 0x3F,
  0x0F, 0x37, 0x11, 0xD5, 0xD4, 0x02, 0xB8, 0xD4, 0x04, 0x73, 0xD5, 0x8C, 0xFD, 0x0A, 0x32, 0x6D,
  0x8C, 0xFD, 0x0D, 0x32, 0x6D, 0x8C, 0xFD, 0x0C, 0x32, 0x68, 0x8C, 0xFD, 0x09, 0x32, 0x5E, 0x8C,
  0xFD, 0x0B, 0x32, 0x63, 0x8C, 0xFD, 0x08, 0x32, 0x59, 0x8C, 0xFD, 0x7F, 0x32, 0x54, 0x8C, 0xFD,
  
  //Address: 0x0440 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x18, 0x32, 0x4F, 0x8C, 0xFD, 0x1F, 0x32, 0x48, 0xD4, 0x03, 0xD9, 0x30, 0x72, 0x30, 0x72, 0xD4,
  0x03, 0xE0, 0x30, 0x72, 0xD4, 0x03, 0xCF, 0x30, 0x72, 0xD4, 0x03, 0x1F, 0x30, 0x72, 0xD4, 0x03,
  0x11, 0x30, 0x72, 0xD4, 0x03, 0x4B, 0x30, 0x72, 0xD4, 0x03, 0xE7, 0x30, 0x72, 0xD4, 0x03, 0x3C,
  0x30, 0x72, 0xD5, 0xD4, 0x04, 0xC6, 0x8E, 0x3A, 0x7C, 0xD4, 0x02, 0xFE, 0x8C, 0xFD, 0x7F, 0x32,
  
  //Address: 0x0480 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0x86, 0x8C, 0xFF, 0x20, 0x33, 0x8B, 0xD4, 0x04, 0x1B, 0x30, 0x9A, 0x8E, 0x73, 0x9E, 0x73, 0xD4,
  0x01, 0x00, 0x60, 0x72, 0xBE, 0xF0, 0xAE, 0xD4, 0x03, 0xB4, 0xD4, 0x04, 0xB5, 0xD5, 0xD4, 0x02,
  0xB8, 0x0F, 0xAC, 0x32, 0xB4, 0x1F, 0x8F, 0x73, 0x9F, 0x73, 0xD4, 0x04, 0x73, 0x60, 0x72, 0xBF,
  0xF0, 0xAF, 0x30, 0xA1, 0xD5, 0xF8, 0x05, 0xAD, 0xF8, 0x7F, 0xBD, 0x8E, 0x5D, 0xF8, 0x06, 0xAD,
  
   //Address: 0x04C0 (Page 4)
  //00    1     2     3     4     5     6     7     8     9     A     B     C     D     E     F   
  0xF8, 0x7F, 0xBD, 0x9E, 0x5D, 0xD5, 0xF8, 0x05, 0xAD, 0xF8, 0x7F, 0xBD, 0x0D, 0xAE, 0xF8, 0x06,
  0xAD, 0xF8, 0x7F, 0xBD, 0x0D, 0xBE, 0xD5, 0xD4, 0x02, 0x9C, 0xD4, 0x02, 0xA7, 0x7B, 0xD4, 0x03,
  0xFA, 0x7A, 0xD4, 0x04, 0x14, 0x30, 0xDD, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  };
...
char getNextByteChar(byte program_type, int offset, byte nibble) {
  ...
  if (nibble < 2 && offset < pgm_page_size) {
    switch (program_type) {
      //Load all zeros
      case PGM_ZEROS:
        if (offset < sizeof(clear_memory)) {              
          pgm_data = pgm_read_byte_near(clear_memory + offset);
          c_byte = getHexDigitChar(pgm_data, nibble);
        } else {
          //Pad with zeros until the end of page
          c_byte = '0';
        } //if-else offset < sizeof clear_memory
      break;
      
      //Load a sequence of digits in the page
      case PGM_SEQUENCE:
        if (offset < sizeof(sequence_memory)) {              
          pgm_data = pgm_read_byte_near(sequence_memory + offset);
          c_byte = getHexDigitChar(pgm_data, nibble);
        } else {
          //Pad with zeros until the end of page
          c_byte = '0';
        } //if-else offset < sizeof sequence_memory
      break;
      ...
      case PGM_GRAPHICS:
        if (offset < sizeof(graphics_demo)) {              
          pgm_data = pgm_read_byte_near(graphics_demo + offset);
          c_byte = getHexDigitChar(pgm_data, nibble);
        } //if offset < sizeof graphics_demo
      break;  
      
      case PGM_TERMINAL:
        if (offset < sizeof(tty_terminal)) {              
          pgm_data = pgm_read_byte_near(tty_terminal + offset);
          c_byte = getHexDigitChar(pgm_data, nibble);
        } //if offset < sizeof tty_terminal
      break;           
